...
class VacuumSearch(BaseModel):
    sample_tags: Optional[str] = Field(Query(...))
...
class TestCreate(TestBase):
    ...
    def as_form(cls,
                category: str = Form(...),
                vac_type: str = Form(...),
                test_status: str = Form("NEW"),
                assigned1: int = Form(...),
                assigned2: Optional[int] = Form(None),
                due_date: Optional[datetime] = Form(None),
                completion_date: Optional[datetime] = Form(None),
...
class VacType(BaseModel):
    vactype: str

    @classmethod
    def as_form(cls,
                vactype: str = Form(...)):
        return cls(vactype=vactype)
